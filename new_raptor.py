...
class NewRaptor:
    ...
    def __init__(self, iface):
        ...
        self.actions = []
        self.menu = self.tr(u'&Add new raport')
    ...
    def run(self):
        ...
        self.dlg.dteLastSurvey.setDate(QDate.currentDate())
        ...
        if missing_layers:
            msg = "The following layers are missing from this projekt\n"
        ...
        if result:
            ...
            valBufferDist = self.dlg.spbBufferDist.value()
            valStatus = self.dlg.cmbStatus.currentText()
            valLast = self.dlg.dteLastSurvey.date()
            QMessageBox.information(self.dlg,'Message','New Nest ID: {}\n\nLatitude: {}\nLongitude: {}\nSpecies: {}\nBuffer: {}\nStatus: {}\n Last survey: {}\n'.format(valNestID,valLat,valLng,valSpecies,valBufferDist,valStatus,valLast))
            ...
            ftrNest.setAttribute('buf_dist',valBufferDist)
            ...
            buffer = geom.buffer(valBufferDist,10)
            ...
            # Find linear projects that will be impacted and report them in table
            bb = buffer.boundingBox()
            linears = lyrLinear.getFeatures(bb)
            for linear in linears:
                valID = linear.attribute('Project')
                valType = linear.attribute('type')
                valDist = linear.geometry().distance(geom)
                if valDist < valBufferDist:
                    ...
                    twi = QTableWidgetItem("{:4.5f}".format(valDist))
    ...
    def evt_cmbSpecies_changed(self,species):
        if species == "Swainsons Hawk":
            self.dlg.spbBufferDist.setValue(0.004)
        else:
            self.dlg.spbBufferDist.setValue(0.008)
